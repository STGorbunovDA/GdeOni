 ...
 *  3. Если точность достигла порога (Geolocation:TargetAccuracyMeters,
 *     дефолт 2 м) — останавливаемся раньше, не ждём остаток окна.
 *  4. По истечении окна отдаём лучший собранный fix.
 *
 * currentAccuracy обновляется по ходу — оверлей показывает «текущая точность».
 *
 * Оговорки:
 *  - Порог 2 м — это реально достижимый на телефоне «хороший» фикс: поймали
 *    его — сразу отдаём, не ждём зря. Но железо телефона на улице обычно даёт
 *    2-10 м, десктоп без GPS — сотни метров (WiFi/IP); порог задаёт лишь МОМЕНТ
 *    остановки, а не саму точность. Поэтому рядом всегда остаётся ручной сдвиг
 *    маркера на карте.
...
const DEFAULT_TARGET_ACCURACY_M = 2;
  ...
  const request = useCallback(() => {
    ...
    watchIdRef.current = navigator.geolocation.watchPosition(
      (pos) => {
        ...
        // Достигли порога (дефолт 2 м) — не ждём остаток окна, сразу отдаём.
        // При слишком малом пороге (0.5/0.2 м) телефон его почти не берёт и
        // сбор шёл бы всё окно; 2 м — реально достижимо, так что срабатывает.
