    ...
    IAuthService authService) : ObservableObject
{
    ...
    private async Task DeleteUserAsync()
    {
        ...
        var confirmed = await page.DisplayAlertAsync(
            ...
            $"Юзер {Email} будет удалён. Его карточки умерших и файлы переуступятся на вас " +
            "(в истории правок зафиксируется переуступка с email удалённого). Платежи и записи в истории правок " +
            "останутся как есть.",
            "Удалить",
            "Отмена");
        if (!confirmed) return;
