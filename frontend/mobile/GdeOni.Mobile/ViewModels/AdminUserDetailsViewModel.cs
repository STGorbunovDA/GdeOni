    ...
    IAuthService authService) : ObservableObject
{
    ...
    private async Task DeleteUserAsync()
    {
        ...
        var confirmed = await page.DisplayAlertAsync(
            ...
            $"Юзер {Email} будет удалён. Карточки умерших, файлы и история правок останутся в системе. " +
            "Если он создавал карточки или загружал файлы — БД не позволит удалить.",
            "Удалить",
            "Отмена");
        if (!confirmed) return;
        ...
        try
        {
            ...
            else
            {
                ErrorMessage = (int)resp.StatusCode switch
                {
                    ...
                    409 => "Юзер создавал карточки умерших или загружал файлы. Удалите или переназначьте этот контент перед удалением аккаунта.",
