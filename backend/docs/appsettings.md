...
Окно сбора «лучшего» GPS-фикса на вебе. При добавлении/правке координат браузер через `watchPosition` собирает замеры это число секунд и берёт самый точный (GPS «дозревает» 10–30 с — первый fix почти всегда худший). Значение отдаётся клиенту в `GET /api/app/features` (`geoAcquireWindowSeconds`), поэтому меняется **без пересборки фронта**: правишь `appsettings.json` → рестарт API → новое окно подхватится на следующей загрузке страницы.

- **`AcquireWindowSeconds`** — длина окна, секунды. Дефолт `60`. На клиент отдаётся с клампом `5..300` (защита от опечатки: `0` = моментальный худший fix, гигантское = человек ждёт вечно). Если клиент не смог прочитать конфиг (аноним/ошибка) — использует свой фолбэк `60`.
