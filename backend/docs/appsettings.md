...
```json
"Geolocation": {
  "AcquireWindowSeconds": 60,
  "TargetAccuracyMeters": 0.5
}
```

Окно сбора «лучшего» GPS-фикса на вебе. При добавлении/правке координат браузер через `watchPosition` собирает замеры это число секунд и берёт самый точный (GPS «дозревает» 10–30 с — первый fix почти всегда худший). Значения отдаются клиенту в `GET /api/app/features` (`geoAcquireWindowSeconds`, `geoTargetAccuracyMeters`), поэтому меняются **без пересборки фронта**: правишь `appsettings.json` → рестарт API → подхватится на следующей загрузке страницы.

- **`AcquireWindowSeconds`** — длина окна, секунды. Дефолт `60`. Кламп `5..300` (защита от опечатки: `0` = моментальный худший fix, гигантское = человек ждёт вечно). Фолбэк клиента (аноним/ошибка) — `60`.
- **`TargetAccuracyMeters`** — порог ранней остановки, метры. Дефолт `0.5`. Как только точность достигла этого значения, сбор прекращается досрочно. Кламп `0..1000` (`0` = не останавливаться досрочно, собирать всё окно). **Важно:** браузерный GPS телефона редко даёт лучше ~3–10 м, поэтому при `0.5`/`0.2` ранняя остановка почти не срабатывает — сбор идёт все N секунд и отдаёт лучший достижимый фикс. Меньшее значение НЕ повышает точность железа, лишь отключает раннюю остановку. Фолбэк клиента — `0.5`.
...
## RelativeDiscovery (Родственники, Фаза 4)
...
Фоновый сервис функции «Родственники»: раз в сутки проходит по всем активным отслеживаниям и находит пары «(владелец, умерший, родственник)», которых ещё нет в `relative_discoveries`. По каждой новой заводит запись-уведомление — она показывается в попапе «События» («у вас новый родственник») и в бейдже вкладки «Родственники». Дедупликация — уникальный индекс `ux_relative_discoveries_owner_deceased_relative`, поэтому повторный прогон/рестарт не дублируют уведомления.

- **`Enabled`** — главный switch. В отличие от `AnniversaryEmails`, внешних зависимостей (SMTP) нет — уведомление внутреннее, поэтому дефолт `true`.
- **`RunAtHourLocal`** — час локального времени (0–23) прогона. 3 — ночью, «вместе с бэкапом БД», чтобы к утру уведомления были свежими.
- **`TimeZoneId`** — часовой пояс для часа прогона (IANA-id). Нераспознанный id → fallback на UTC с warning.
- **`MaxJitterSeconds`** — случайный разброс старта (защита от одновременного старта нескольких реплик). `0` — без джиттера.

Непрочитанные сообщения от родственников в бейдж/попап считаются **вживую** (из переписки), ночной джоб их не касается.

**Миграция:** таблица `relative_discoveries` добавляется миграцией `AddRelativeDiscoveries` — накатить `dotnet ef database update` перед стартом (иначе fail-fast на pending-миграции).

---

## EmailConfirmation (D45)

```json
"EmailConfirmation": {
  "TokenLifetimeHours": 48,
  "WebConfirmUrl": "https://gdeoni.ru/confirm-email",
  "AppName": "Где Они"
}
```

Подтверждение адреса email по ссылке из письма. **Гейт для новых пользователей**: пока адрес не подтверждён, `/auth/login` не пускает (и веб, и мобилка — обе ходят на один эндпоинт). «Старые» пользователи (зарегистрированные до фичи) под гейт не попадают — у них только баннер «Подтвердите email» внутри приложения.

- **`WebConfirmUrl`** — базовый адрес страницы подтверждения на сайте, к нему добавляется `?token=...`. **Должен совпадать с роутом `/confirm-email` в вебе.** Пустое значение (или пустая секция) **выключает** фичу: письма не шлются и **гейт входа не применяется** — приложение работает как раньше (важно для dev без SMTP).
- **`TokenLifetimeHours`** — сколько живёт ссылка. 48 ч: подтверждение читают не так срочно, как сброс пароля.
- **`AppName`** — подпись в письме.

**Условие срабатывания гейта:** канал должен быть **реально готов** — задан `WebConfirmUrl` **И** сконфигурирована секция `Email` (SMTP). Если письма слать нечем, гейт отключается автоматически (иначе новых юзеров было бы нечем разблокировать; заодно это спасает dev/интеграционные тесты без SMTP от тотального локаута).

- Эндпоинты: `POST /auth/confirm-email` (по токену), `POST /auth/resend-confirmation` (повторная отправка, анонимный, всегда 200 — как forgot-password, чтобы не палить существование аккаунта). Оба под rate-limit политики `auth`.
- Код ошибки гейта на логине: `user.email.not_confirmed` (403). Клиент по нему показывает экран «проверьте почту» с кнопкой повторной отправки.

**Миграция:** колонки подтверждения добавляются миграцией `AddEmailConfirmation`. Бэкфилла нет: существующие строки через дефолты получают `is_email_confirmed=false` + `email_confirmation_required=false` (баннер есть, гейта нет) — ровно нужное «старым» юзерам. Накатить `dotnet ef database update` перед стартом.

**Замечание про сид-админа:** если SuperAdmin уже существовал до миграции, после неё он получит баннер (не подтверждён), но под гейт не попадёт (required=false) — вход не блокируется. Впервые сидируемый админ создаётся сразу подтверждённым.
...
«Поделиться подборкой карточек»: отправитель выбирает свои отслеживаемые карточки → короткая ссылка `gdeoni.ru/s/<код>` + QR → получатель входит и добавляет карточки себе в отслеживание.

- **`LinkLifetimeHours`** — сколько живёт ссылка/QR. 24 ч по умолчанию.

Секция **опциональна**: без неё используется дефолт (24 ч). Никаких секретов и внешних сервисов не требует. Полную ссылку/QR веб-клиент строит сам от своего origin — отдельного «base URL» в конфиге не нужно.

**Миграция:** таблица `share_bundles` (короткий код → `deceased_ids uuid[]`) добавляется миграцией `AddShareBundles` — накатить `dotnet ef database update` перед стартом.

**Приватности:** публичной страницы нет — и раскрытие подборки, и импорт требуют входа. Импорт добавляет карточки в отслеживание (под подпиской; новый юзер на триале — сразу).
